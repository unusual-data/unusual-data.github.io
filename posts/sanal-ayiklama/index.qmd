...
image: "/projects/virtual-sorting/images/factory-io.jpg"
toc : true
toc-depth: 9
---
...
![](/projects/virtual-sorting/images/factory-io.jpg){.lightbox group="factory-io"}
...
```{.python title="Konfigürasyon ve Başlatma"}
...
```{.python title="Algılama ve Sınıflandırma Aşaması"}
with mss.mss() as sct:
    while True:
...
```{.python title="PLC Bit Güncelleme"}
...
```{.python title="Periyodik Durum Log"}
        now = time.time()
        if now - last_log > 0.5:
...
```{.python title="Debug Amaçlı Görüntü Gösterimi"}
        if show_debug:
...
```{.python title="Döngü Zamanlama Kontrolü"}
...
```{.python title="Uygulama Sonlandırma Aşaması"}
...
```{.python title="Python Kodunun Tamamını Görüntülemek için Tıklayınız"}
import time
import cv2
import numpy as np
import mss
...
{{< video /projects/virtual-sorting/videos/virtual-sorting.mp4 >}}